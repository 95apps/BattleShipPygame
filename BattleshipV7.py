...
mainMenu = Sprite("resources/images/MainMenu.png", cellSize[0] * 9, 0, int(cellSize[0]) * 8, int(cellSize[1]) * 16)
mainMenuButtons = [MenuButton("newgame", "resources/images/NewGameButton.png", cellSize[0] * 9, cellSize[1] * 3, int(cellSize[0] * 8), int(cellSize[1] * 2)),
                   ...
                   MenuButton("load", "resources/images/LoadButton.png", cellSize[0] * 9, cellSize[1] * 9, int(cellSize[0] * 8), int(cellSize[1] * 2)),
                   MenuButton("quit", "resources/images/QuitButton.png", cellSize[0] * 9, cellSize[1] * 12, int(cellSize[0] * 8), int(cellSize[1] * 2))]
...
while gameLoop:
    ...
    for y in range(0, 10):
        for x in range(0, 10):
            ...
            if gameState == 2:
                ...
                if opponentRect.collidepoint((mouseX, mouseY)) and opponentMatrix[y][x]["isHit"] == False:
                    opponentMatrix[y][x]["isHit"] = True
                    targetingCell = True
                    targetedCell = (y, x)

            if playerMatrix[y][x]["isHit"]:
                if playerMatrix[y][x]["hasShip"]:
                    pygame.draw.circle(screen, (255, 0, 0, 255), playerRect.center, int(cellSize[0] / 2))
                else:
                    pygame.draw.circle(screen, (255, 255, 255, 255), playerRect.center, int(cellSize[0] / 2))
    ...
    elif gameState == 3 and targetingCell:
        gameState = 4
    elif gameState == 4:
        gameState = 3
